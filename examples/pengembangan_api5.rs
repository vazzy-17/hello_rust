...
async fn main() -> tide::Result<()> {
    ...
    app.at("/api/log_user").post(get_log_user);
...
async fn get_log_user(mut req:Request<PgPool>)-> tide::Result{
    let body_json=req.body_json::<LogReques>().await;
    let body=match body_json{
        Ok(b)=>b,
        Err(_)=>{
            let mut res=Response::new(StatusCode::BadRequest);
    ...
    match sqlx::query_as::<_, Userlog>("select msisdn,message,source,ts from log_archive where msisdn=$1")
    .bind(&body.no_user)
    .fetch_all(pool)
    .await
    {
        ...
        Err(sqlx::Error::RowNotFound)=>{
            let mut res=Response::new(StatusCode::NotFound);
            res.set_body(Body::from_string(format!("Number {} not found",body.no_user)));
            Ok(res)
        },
        Err(e)=>{
            eprintln!("Database error: {:?}",e);
